...
import type { AboutExperienceItem, AboutToolItem } from '@/lib/sanity-queries'
...
export function AboutPageContent({ data }: { data: AboutPageData }) {
  ...
  // Split "Ready to Collaborate?" into leading text + emphasized last word, matching original design
  const ctaWords = data.ctaHeadline.trim().split(' ')
  const ctaLastWord = ctaWords.pop() || ''
  const ctaLeading = ctaWords.join(' ')
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-16 xl:px-20">
          <div style={{ overflow: 'hidden' }} className="mb-10 lg:mb-14">
            ...
            >
              ...
              <span className="text-[10px] uppercase tracking-[0.32em] text-primary font-semibold">{data.eyebrow}</span>
            </motion.div>
          </div>

          <h1 className="font-black leading-[0.9] tracking-[-0.03em] mb-14 lg:mb-20">
            <div className="text-[clamp(3.5rem,9vw,8rem)] text-white/90">
              <SplitChars text={data.headline.replace(/\.$/, '')} delay={0.1} />
            </div>
            {data.headline.endsWith('.') && (
              <div className="text-[clamp(3.5rem,9vw,8rem)]">
                <SplitChars text="." delay={0.25} className="text-primary" />
              </div>
            )}
          </h1>
      ...
      <section className="bg-surface border-b border-border">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-16 xl:px-20 py-24 lg:py-32">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-14">
            ...
            >
              {String(data.skills.length).padStart(2, '0')} Disciplines
            </motion.p>
          </div>
        ...
        <div ref={ctaRef} className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-16 xl:px-20 py-28 lg:py-44">
          <div className="max-w-3xl">
            ...
            <div style={{ overflow: 'hidden' }} className="mb-10">
              ...
              >
                {ctaLeading}{' '}
                <span className="text-primary italic">{ctaLastWord}</span>
              </motion.h2>
            </div>
            ...
            >
              <a
                href={`mailto:${data.ctaEmail}?subject=Let's%20Collaborate!&body=Hi%20Hesham%2C%0D%0A%0D%0AMy%20name%20is%20[Your%20Name].%20I'm%20reaching%20out%20to%20discuss%20a%20potential%20collaboration%20or%20project.%20Here%20are%20some%20details%3A%0D%0A%0D%0A---%0D%0A%0D%0AThank%20you!`}
